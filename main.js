...
// Intentamos recuperar el usuario de la memoria
let user = JSON.parse(localStorage.getItem('rani_user')) || null;

document.addEventListener('DOMContentLoaded', () => {
    // Si el usuario ya existe en memoria, mostramos la app directamente
    if (user) {
        mostrarApp();
    }
    // Siempre cargamos los partidos
    cargarFixture();
});

// --- LÓGICA DE GOOGLE ---

// Esta función es la que Google llama automáticamente al loguearse exitosamente
function handleCredentialResponse(response) {
    // Decodificamos el "Token" seguro que nos manda Google
    const data = parseJwt(response.credential);

    // Creamos el perfil del usuario con su info real de Google
    user = {
        id: data.sub, // ID único de Google
        name: data.given_name || data.name.split(' ')[0], // Primer nombre
        photo: data.picture, // URL de su foto de perfil
    ...
    // Cambiamos la interfaz para mostrar su info
    mostrarApp();
}

// Función mágica necesaria para leer los datos encriptados de Google
...
function cargarFixture() {
    ...
    if (!container) return; // Seguridad por si acaso
    container.innerHTML = "";
...
function canjearCodigo() {
    ...
    if (!user) return alert("Inicia sesión primero con Google para proteger tus monedas.");
